              ...
              <div className="px-2 pt-2 pb-3 space-y-1">
                {navItems.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="block px-3 py-2 text-[#999999] hover:text-[#3ebf7e] font-medium text-left"
                    onClick={() => setIsOpen(false)}
                  >
                    {item.label}
                  </Link>
                ))}
                {/* Force language switcher left-aligned */}
                <div className="px-0 py-2 flex justify-start">
                  {locale != "en" && <div className="w-full "></div>}
                  <LanguageSwitcher
                    currentLocale={locale}
                    scrolled={scrolled}
                  />
